package com.ankamagames.dofus.misc.stats.custom
{
   ...
   public class ShortcutsStats implements IHookStats, IStatsClass
   {
      ...
      public function onHook(pHook:Hook, pArgs:Array) : void
      {
         ...
         if(pHook.name == CustomUiHookList.ShowTheoreticalEffects.name)
         {
            this._keyboardShortcut = "showTheoreticalEffects";
            this.onNoUiShortcut(this._keyboardShortcut);
         }
         else if(pHook.name == CustomUiHookList.FoldAll.name)
         {
            this._keyboardShortcut = "foldAll";
            this.onNoUiShortcut(this._keyboardShortcut);
         }
         else if(pHook.name == BeriliaHookList.UiLoading.name)
         {
      ...
      public function process(pMessage:Message, pArgs:Array = null) : void
      {
         ...
         {
            BindsManager.getInstance().registerEvent(new GenericListener("ALL","ShortcutsStats",this.listener,int.MAX_VALUE));
            EnterFrameDispatcher.addEventListener(this.onEnterFrame,"ShortcutsStats");
         }
