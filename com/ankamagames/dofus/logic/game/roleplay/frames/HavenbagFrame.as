         ...
         switch(true)
         {
            ...
            case msg is HavenBagDailyLoteryMessage:
               ...
               if(hbdlmsg.returnType == HavenBagDailyLoteryErrorEnum.HAVENBAG_DAILY_LOTERY_OK && this._lotteryApi.hasOwnProperty("consumeByIdApiCall"))
               {
                  ...
                  {
                     _lotteryApi.getApiClient().setApiKey(apiKey);
                     _lotteryApi.consumeByIdApiCall(XmlConfig.getInstance().getEntry("config.lang.current"),parseFloat(hbdlmsg.gameActionId),GameID.current);
                  });
               }
               else
               {
