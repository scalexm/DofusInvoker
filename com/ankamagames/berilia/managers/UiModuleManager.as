package com.ankamagames.berilia.managers
{
   ...
   public class UiModuleManager
   {
      ...
      private var _modulesEmbeddedSWFs:Dictionary;
      ...
      public function set modulesEmbeddedSWFs(v:Dictionary) : void
      {
         this._modulesEmbeddedSWFs = v;
      }
      ...
      public function init(filter:Array, filterInclude:Boolean) : void
      {
         ...
         var file:File = null;
         if(this._modulesEmbeddedSWFs == null)
         {
            ErrorManager.addError("Property \'_modulesEmbeddedSWFs\' of UiModuleManager must be set before calling init method");
         }
         if(this._everyModuleNames == null)
         {
            ...
            if(this._modulesRoot.exists)
            {
               ...
               {
                  if(file.isDirectory && !this._modulesEmbeddedSWFs[file.name])
                  {
      ...
      public function reenableModule(id:String) : void
      {
         ...
         this._modules[id] = uiModule;
         uiModule.bindUiClassesFromSWFApplicationDomain(uiModule.applicationDomain);
         uiModule.mainClass = uiModule.loader.content;
      ...
      private function onDMOrD2UILoad(e:ResourceLoadedEvent) : void
      {
         ...
         var moduleLoader:Loader = null;
         var scriptClass:Class = null;
         var shortcutsUri:Uri = null;
         var ui:UiData = null;
         var uiUri:Uri = null;
         _log.warn("Start load of module : " + e.uri);
         ...
         _log.warn("UIModule created : " + e.uri);
         this._unInitializedModules.push(um);
         scriptClass = this._modulesEmbeddedSWFs[um.id];
         if(scriptClass == null)
         {
         ...
         _log.warn("Module is Embed " + e.uri);
         ...
         um.trusted = true;
         moduleLoader.contentLoaderInfo.addEventListener(Event.COMPLETE,function(e:Event):void
         {
            onModuleScriptLoaded(e,um);
         });
         if(um.shortcuts)
         {
            _log.warn("Module has shortcuts : " + e.uri);
         ...
         {
            moduleLoader.loadBytes(scriptBytes,lc);
            _log.warn("No shortcuts for module : " + e.uri + "   loadbytes finished");
         }
         if(e.resourceType == ResourceType.RESOURCE_XML)
         {
         ...
         _log.warn("DmOrD2UI function finished for : " + e.uri);
      }
      
      private function onModuleScriptLoaded(e:Event, uiModule:UiModule) : void
      {
         ...
         _log.trace("Load script " + uiModule.id + ", " + (this._moduleCount - this._scriptNum + 1) + "/" + this._moduleCount);
         uiModule.loader = l;
         uiModule.bindUiClassesFromSWFApplicationDomain(l.contentLoaderInfo.applicationDomain);
      ...
      private function onUiLoaded(e:ResourceLoadedEvent) : void
      {
         ...
         for(xml = e.resource as String; res = _regImport.exec(xml); )
         {
      ...
      {
         _log.warn("onShortcutsLoad start for : " + e.uri);
         ...
         _log.warn("onShortcutsLoad finished for : " + e.uri);
      }
      ...
      {
         var module:UiModule = null;
         var unparsedXml:Vector.<UiData> = null;
         var url:* = null;
         var ui:UiData = null;
         ...
         for(url in this._xmlFilesToParse)
         {
            ...
            if(uiDataList[url])
            {
               unparsedXml.push(uiDataList[url]);
            }
         }
         this.parseNextXml(unparsedXml);
      }
      ...
      private function parseNextXml(unparsedXml:Vector.<UiData>) : void
      {
         ...
         else
         {
            ...
            xmlParsor.addEventListener(Event.COMPLETE,function(e:ParsorEvent):void
            {
               onXmlParsed(e,unparsedXml);
            },false,0,true);
            xmlParsor.addEventListener(ParsingErrorEvent.ERROR,function(e:ParsingErrorEvent):void
            {
               onXmlParsingError(e,unparsedXml);
            });
            xmlParsor.processFile(uiData.file);
         }
      }
      
      private function onXmlParsed(e:ParsorEvent, unparsedXml:Vector.<UiData>) : void
      {
         if(e.uiDefinition)
         {
      ...
      private function onXmlParsingError(e:ParsingErrorEvent, unparsedXml:Vector.<UiData>) : void
      {
